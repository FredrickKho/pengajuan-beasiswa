                ...
                @if (!$dataBeasiswa->isEmpty())
                    ...
                    <table class="table table-striped table-bordered border-black my-3 text-center">
                        ...
                        @foreach ($dataBeasiswa as $data)
                            <tr>
                                ...
                                @php
                                    ...
                                    if($data->pengajuanBeasiswa->isApprovedByProgramStudi){
                                        ...
                                        $color = "btn-secondary";
                                        $text = "Waiting program studi to review and approve";
                                        $disabled = "disabled";
                                    }
                                @endphp
                                <th>
                                    <button data-bs-toggle="modal" data-bs-target="#review-{{$loop->index+1}}" style="font-size: small" class="btn {{$color}}" {{$disabled}}>{{$text}}</button>
                                    @if ($data->pengajuanBeasiswa->isApprovedByDekan)
                                        <button data-bs-toggle="modal" data-bs-target="#finalize-{{$loop->index+1}}" style="font-size: small" class="btn btn-success">Finalisasi</button>
                                    @endif
                                </th>
                            </tr>
                                ...
                                <div class="modal-dialog modal-dialog-centered" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body d-flex flex-column">
                                            ...
                                            <form id="reviewForm{{$data->id}}" action="{{route('dekanReviewBeasiswa',$data->id)}}" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                @method('PUT')
                            ...
                            <!-- Modal finalisasi-->
                            <div class="modal fade" id="finalize-{{$loop->index+1}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="exampleModalLongTitle">Finalisasi beasiswa</h5>
                                        </div>
                                        <div class="modal-body">
                                            <div>
                                                Apakah anda ingin melakukan finalisasi beasiswa tersebut?
                                            </div>
                                            <strong class="text-danger">
                                                CATATAN: DENGAN FINALISASI BEASISWA BERATI FAKULTAS DAN PROGRAM STUDI TELAH MENYETUJUI PENGAJUAN BEASISWA TERSEBUT DAN TIDAK DAPAT DIREVIEW KEMBALI.
                                            </strong>
                                        </div>
                                        <div class="modal-footer">
                                            <form id="reviewForm{{$data->id}}" action="{{route('finalize',$data->id)}}" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                @method('PUT')
                                                <button type="submit" class="btn btn-success" data-bs-dismiss="modal">Finalisasi</button>
